...
def linreg():
        if request.method=='POST':
                #get x and y values from template
                if request.form['x_vals']:
                        x_vals=request.form['x_vals'].split(',')
                if request.form['y_vals']:
                        y_vals = request.form['y_vals'].split(',')
                #determine which array is shorted, x or y. We will only use the number of data points in the smaller array
                if len(x_vals)<len(y_vals):
                        smaller_len = len(x_vals)
                else:
                        smaller_len = len(y_vals)
